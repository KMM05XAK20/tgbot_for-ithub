...
def collect_admin_stats() -> dict:
    ...
    with SessionLocal() as s:
        ...
        assignments_active = s.query(TaskAssignment).filter(TaskAssignment.status == "active").count()
        assignments_submitted = s.query(TaskAssignment).filter(TaskAssignment.status == "submitted").count()
        assignments_approved = s.query(TaskAssignment).filter(TaskAssignment.status == "approved").count()
        assignments_rejected = s.query(TaskAssignment).filter(TaskAssignment.status == "rejected").count()
