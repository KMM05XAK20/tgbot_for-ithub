...
from ..services.mentorship import create_mentor_application, get_incoming_for_mentor, get_mentor_list, get_user_applications, set_application_status
from ..storage.models import MentorTopic  # Добавляем импорт для MentorTopic
from ..keyboards.common import (mentorship_root_kb, mentor_list_kb, mentor_topics_kb,
                                mentor_confirm_kb, mentor_inbox_kb, main_menu_kb)
...
async def mentorship_root(cb: CallbackQuery):
    ...
    await cb.message.edit_text(text, reply_markup=mentorship_root_kb(), parse_mode=ParseMode.HTML)
    await cb.answer()
...
async def choose_mentor(cb: CallbackQuery):
    mentors = get_mentor_list()
    if not mentors:
        await cb.message.edit_text("Пока нет доступных наставников.", reply_markup=mentorship_root_kb())
        return await cb.answer()
    await cb.message.edit_text("Выберите наставника:", reply_markup=mentor_list_kb(mentors))
    await cb.answer()
...
async def confirm_application(cb: CallbackQuery):
    ...
    if app.status == "pending":
        ...
        await cb.message.edit_text("Заявка обновлена.", reply_markup=mentorship_root_kb())
    await cb.answer()
...
async def my_apps(cb: CallbackQuery):
    ...
    if not apps:
        await cb.message.edit_text("У вас пока нет заявок.", reply_markup=mentorship_root_kb())
        return await cb.answer()
    lines = []
    for a in apps:
        topic = a.topic
...
async def mentor_inbox(cb: CallbackQuery):
    ...
    if not inbox:
        await cb.message.edit_text("Входящих заявок нет.", reply_markup=mentorship_root_kb())
        return await cb.answer()
    ...
    text = (f"📥 Заявка #{app.id}\n"
            ...
            f"Статус: {app.status}")
    await cb.message.edit_text(text, reply_markup=mentor_inbox_kb(app.id), parse_mode=ParseMode.HTML)
    await cb.answer()
...
async def app_decision(cb: CallbackQuery):
    ...
    updated = set_application_status(app_id, cb.from_user.id, "approved" if action == "approve" else "rejected")
    if not updated:
    ...
    if usr and usr.tg_id:
        try:
            ...
            pass
    await cb.message.edit_text(f"Заявка #{updated.id}: статус → {updated.status}", reply_markup=mentorship_root_kb())
    await cb.answer()
