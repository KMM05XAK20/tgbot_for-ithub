...
from ..storage.models import MentorTopic, TaskAssignment
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="👤 Профиль", callback_data="menu:open:profile"
                )
            ],
            [
                InlineKeyboardButton(
                    text="📚 Каталог заданий", callback_data="menu:open:tasks"
                )
            ],
            [InlineKeyboardButton(text="🏆 Рейтинг", callback_data="menu:open:rating")],
            [
                InlineKeyboardButton(
                    text="🤝 Менторство", callback_data="menu:open:mentorship"
                )
            ],
            [
                InlineKeyboardButton(
                    text="🗓 Календарь", callback_data="menu:open:calendar"
                )
            ],
...
def admin_panel_kb() -> InlineKeyboardMarkup:
    ...
    # Первый ряд
    kb.button(text="📋 Задания", callback_data="admin:tasks")
    kb.button(text="👥 Менторы", callback_data="admin:mentors")
    ...
    # Отдельным рядом — выход в меню
    kb.button(text="⬅️ В меню", callback_data="menu:open:main")
    kb.adjust(2, 1)
...
def admin_events_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="➕ Добавить событие", callback_data="admin:events:add"
                )
            ],
...
def admin_grant_kb(user_id: int):
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🛡 Выдать админку", callback_data=f"admin:grant:{user_id}"
                )
            ]
        ]
    )
...
def admin_mentors_root_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="➕ Добавить ментора", callback_data="admin:mentors:add"
                )
            ],
            [
                InlineKeyboardButton(
                    text="🗑 Удалить ментора", callback_data="admin:mentors:remove"
                )
            ],
            [
                InlineKeyboardButton(
                    text="📋 Список менторов", callback_data="admin:mentors:list"
                )
            ],
...
def admin_tasks_root_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="➕ Добавить задание", callback_data="admin:tasks:add"
                )
            ],
            [
                InlineKeyboardButton(
                    text="📋 Список заданий", callback_data="admin:tasks:list"
                )
            ],
            [
                InlineKeyboardButton(
                    text="🌱 Засеять демо", callback_data="admin:tasks:seed"
                )
            ],
            [InlineKeyboardButton(text="⬅️ Назад", callback_data="admin:panel")],
        ]
...
def admin_review_root_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🕒 На проверке", callback_data="admin:review:pending"
                )
            ],
            [InlineKeyboardButton(text="⬅️ Назад", callback_data="admin:panel")],
        ]
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="✅ Принять",
                    callback_data=f"admin:review:{assignment_id}:approve",
                )
            ],
            [
                InlineKeyboardButton(
                    text="❌ Отклонить",
                    callback_data=f"admin:review:{assignment_id}:reject",
                )
            ],
            [
                InlineKeyboardButton(
                    text="⬅️ Назад", callback_data="admin:review:pending"
                )
            ],
        ]
    )
...
def admin_tasks_list_kb(tasks: list) -> InlineKeyboardMarkup:
    ...
    for t in tasks:
        ...
        rows.append(
            [InlineKeyboardButton(text=label, callback_data=f"admin:tasks:nop:{tid}")]
        )
        ...
        rows.append(
            [
                InlineKeyboardButton(
                    text=("🔓 Опубл." if pub else "🔒 Скрыто"),
                    callback_data=f"admin:tasks:toggle:{tid}",
                ),
                InlineKeyboardButton(
                    text="🗑 Удалить", callback_data=f"admin:tasks:delete:{tid}"
                ),
            ]
        )
...
def profile_history_filters_kb(counts: dict[str, int]) -> InlineKeyboardMarkup:
    ...
    kb.button(
        text=f"🕒 На проверке ({s})", callback_data="profile:history:list:submitted:1"
    )
...
def profile_history_list_kb(
    group: str, page: int, diff: str = "all"
    ...
    kb.button(
        text=chip("Все", "all"),
        callback_data=f"profile:history:list:{group}:{page}:all",
    )
    kb.button(
        text=chip("🟢", "easy"),
        callback_data=f"profile:history:list:{group}:{page}:easy",
    )
    kb.button(
        text=chip("🟡", "medium"),
        callback_data=f"profile:history:list:{group}:{page}:medium",
    )
    kb.button(
        text=chip("🔴", "hard"),
        callback_data=f"profile:history:list:{group}:{page}:hard",
    )
    kb.adjust(4)
    ...
    kb.button(
        text="⬅️",
        callback_data=f"profile:history:list:{group}:{max(1, page - 1)}:{diff}",
    )
...
def mentorship_root_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="👤 Выбрать наставника", callback_data="mentor:choose"
                )
            ],
...
def mentor_role_kb(tg_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="👤 Гуру", callback_data=f"admin:mentors:setrole:{tg_id}:guru"
                )
            ],
            [
                InlineKeyboardButton(
                    text="🧰 Помогатор",
                    callback_data=f"admin:mentors:setrole:{tg_id}:helper",
                )
            ],
...
def mentor_list_kb(mentors: list) -> InlineKeyboardMarkup:
    ...
    for m in mentors:
        ...
        rows.append(
            [InlineKeyboardButton(text=title, callback_data=f"mentor:pick:{m.id}")]
        )
    rows.append(
        [InlineKeyboardButton(text="⬅️ Назад", callback_data="menu:open:mentorship")]
    )
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def mentor_topics_kb(mentor_id: int) -> InlineKeyboardMarkup:
    rows = [
        [
            InlineKeyboardButton(
                text="🎯 Карьера",
                callback_data=f"mentor:topic:{mentor_id}:{MentorTopic.CAREER.value}",
            )
        ],
        [
            InlineKeyboardButton(
                text="📱 Контент",
                callback_data=f"mentor:topic:{mentor_id}:{MentorTopic.CONTENT.value}",
            )
        ],
        [
            InlineKeyboardButton(
                text="🔧 Проекты",
                callback_data=f"mentor:topic:{mentor_id}:{MentorTopic.PROJECTS.value}",
            )
        ],
        [
            InlineKeyboardButton(
                text="💡 Идеи",
                callback_data=f"mentor:topic:{mentor_id}:{MentorTopic.IDEAS.value}",
            )
        ],
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"mentor:confirm:{mentor_id}:{topic}",
                )
            ],
            [
                InlineKeyboardButton(
                    text="⬅️ Назад", callback_data=f"mentor:topic_back:{mentor_id}"
                )
            ],
        ]
    )
...
def mentor_inbox_kb(app_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="✅ Принять", callback_data=f"mentor:app:{app_id}:approve"
                )
            ],
            [
                InlineKeyboardButton(
                    text="❌ Отклонить", callback_data=f"mentor:app:{app_id}:reject"
                )
            ],
...
def tasks_list_kb(tasks: list) -> InlineKeyboardMarkup:
    ...
    for t in tasks:
        ...
        rows.append(
            [
                InlineKeyboardButton(
                    text=f"{title} • {reward}💰", callback_data=f"tasks:view:{tid}"
                )
            ]
        )
    rows.append(
        [InlineKeyboardButton(text="⬅️ Фильтры", callback_data="menu:open:tasks")]
    )
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def task_submit_kb(task_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="📤 Сдать задание", callback_data=f"tasks:submit:{task_id}"
                )
            ],
            [
                InlineKeyboardButton(
                    text="⬅️ Назад к каталогу", callback_data="menu:open:tasks"
                )
            ],
        ]
    )
...
def task_details_kb(task_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="✅ Взять задание", callback_data=f"tasks:take:{task_id}"
                )
            ],
            [
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    text="ℹ️ Подробнее", callback_data=f"tasks:take:{task_id}"
                )
            ],
            [
                InlineKeyboardButton(
...
def get_assignment_card(assignment_id: int) -> tuple[str, InlineKeyboardMarkup] | None:
    ...
    with SessionLocal() as s:
        ...
        uname = (
            ...
            else str(getattr(user, "tg_id", ta.user_id))
        )

        status = ta.status
        submitted_at = (
            ta.submitted_at.strftime("%Y-%m-%d %H:%M") if ta.submitted_at else "—"
        )
